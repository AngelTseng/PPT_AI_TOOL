...
- agenda
- section
- content_2_a / content_2_b / content_2_c
- content_3extra_a / content_3extra_b / content_3extra_image
- content_image
- content_4_a / content_4_b
- table
- flow
- end
...
- Do not repeat the same slide type more than 2 times in a row.
...
- Do not insert a section slide only to break repetition.
- A section slide must reflect a real topic transition.
- Do not create fake table rows or fake flow steps just to satisfy layout diversity.
...
- If the deck contains multiple 3-card slides, distribute them across
  content_3extra_a, content_3extra_b, and content_3extra_image when appropriate.
- Use content_3extra_image only when the slide content clearly includes or benefits from an image.
- Prefer content_3extra_a as the default 3-card text layout.
- Use content_3extra_b to reduce monotony when another 3-card text slide appears nearby.
...
def beautify_spec(extracted_spec: dict) -> dict:
    ...
    layout_warnings = check_layout_streaks(spec, max_streak=2)
...
def check_layout_streaks(spec: dict, max_streak: int = 2) -> list[str]:
    slides = spec.get("slides", [])
    warnings = []
    ...
    for i in range(1, len(slides)):
        ...
        if streak > max_streak:
            warnings.append(
                f"Slide type '{curr_type}' repeats more than {max_streak} times in a row "
                f"(ending at slide {i + 1})."
            )
